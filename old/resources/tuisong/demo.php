...
//define('APPKEY','0XI0ILqMct9owVpQcQfSq5');
//define('APPID','KDxjEfqkmB7GZiMvYKkTK');
//define('MASTERSECRET','1h3AOuKiVG95T8WucYJD94');
define('APPKEY','0CIsRY0qHQ89qRPnKVLzN2');
define('APPID','nLiKcBSk0aA2eDXEI0fIj8');
define('MASTERSECRET','LoFziYsDsI8ynXTo7F2JJ5');
...
//多推接口案例
function pushMessageToList($content,$demo_cid)
{
...
function IGtNotyPopLoadTemplateDemo($title,$content){
        ...
        $template ->set_notyIcon("$_SERVER[DOCUMENT_ROOT] .'/resources/tuisong/ic_launcher.png");//通知栏logo
...
function IGtLinkTemplateDemo(){
        ...
        $template ->set_appId(APPID);//应用appid
        $template ->set_appkey(APPKEY);//应用appkey
        $template ->set_title("123");//通知栏标题
        $template ->set_text("456");//通知栏内容
...
function IGtNotificationTemplateDemo($title,$content){
        ...
        $template->set_logo("http://www.jylc8.com:8081/resources/tuisong/ic_launcher.png");//通知栏logo
        $template->set_logoURL("http://www.jylc8.com:8081/resources/tuisong/ic_launcher.png");
        $template->set_isRing(true);//是否响铃
        $template->set_isVibrate(true);//是否震动
        $template->set_isClearable(true);//通知栏是否可清除
        //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
...
function IGtTransmissionTemplateDemo($title,$content){
        ...
        $template->set_transmissionContent('{"title":"title","content":"'.$content.'"}');//透传内容
        //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
    //APN简单推送
//        $apn = new IGtAPNPayload();
//        $alertmsg=new SimpleAlertMsg();
