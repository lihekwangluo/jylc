...
class WorkmateAction extends IndexAction{
	...
	*/
	public function workmatePublish(){
		...
		$Public = D('Public');
			$data['content'] = $Public->delSensitivity(I('post.content')?I('post.content'):I('get.content'));//详细要求
			...
			if(empty($token)){
				...
				if(!empty($_FILES)){
					...
					for($i=0;$i<=count($_FILES)-1;$i++) {
						array_push($litpic,$Public->Litpic2($imgArr['pic'.$i]));
						$pic .= $imgArr['pic'.$i].',';
					}
	...
	public function workmateList($token,$status,$wid=''){
			...
			$id = I('post.wid')?I('post.wid'):I('get.wid');
				...
				if($status==1){
					...
					$list = $Workmate->sFriendWorkmate(0, $limit, $status, $id);
				}else{
					$list = $Workmate->sFriendWorkmate($uerContent['0']['id'], $limit, $status, $id);
				}
				if (!empty($list)) {
