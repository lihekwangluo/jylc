...
class RecordAction extends Action{
				...
				foreach ($mobile as $k => $v) {
					if(isMobile($v)){
						...
						if(empty($cRelation)){
							...
							get_obj_array('20000',"已经添加过了",'');
						}
					}else{
						get_obj_array('20000',"手机号格式不正确",'');
					}
				}
				$root = array(
								'token'=>$this->token,
				...
				if($this->status <= 3){ 
					...
					if(!empty($this->price)){
						if($this->status == 3){
							...
							$data['price'] = $this->price*$this->square;
						}else{
	...
	public function recordDel(){
		$recordid = I('post.recordid')?I('post.recordid'):I('get.recordid');
		$Record = D('Record');
		$Record->where("id='$recordid'")->delete();
		get_obj_array('10000',"删除成功！",'');
	}

	public function relationOneReset(){
		$recordid = I('post.recordid')?I('post.recordid'):I('get.recordid');
		$Record = D('Record');
		$Record->query("UPDATE `zj_record` SET  `price` = '0',`jprice` = '0',`square` = '',`overtime` = '' WHERE `id` ='$recordid'; ");
		get_obj_array('10000',"清除成功！",'');
		}
