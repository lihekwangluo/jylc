<?php
// 发布商品
class DemandAction extends IndexAction{
...
//		}
		...
		if(empty($data['username']) || empty($data['moblie']) || empty($data['power'])){
            ...
            if(empty($type) && $type !==0 && $type !=='0' && $status == 5){
                $PublishF = M("Alcor");
		        $list = $PublishF->where("id = $alcorid ")->find();
                if(isset($list['ptype'])){
                    $type = $list['ptype'] == 1?2:1;
                }else{
                    get_obj_array('20000','无法找到该经营范围！','');
                }
            }
            $data['type'] = $type;
			if(empty($pid)){
	...
	public function rentOut(){
        ...
        $shopname = I('post.owner')?I('post.owner'):I('get.owner');//设备出租时的业主名
	...
	*/
	...
	public function autoTitle(){
        ...
        $owner = I('post.owner')?I('post.owner'):I('get.owner');//设备出租时的业主名
		$status = intval(I('post.status')?I('post.status'):I('get.status'));
		...
		if($area1){
            ...
            $craft_list = $Craft->where(array('id'=>array('IN',$craftids)))->select();
            $crafts = array();
	...
	public function publishHome(){
		...
		foreach ($variable as $key => $value) {
			$sql = "SELECT p.*,u.id as uid,u.rterrace,u.renzheng,u.type,u.hxname FROM zj_publish as p
...
/*
	public function publishList1(){
		...
		if(!empty($userid)){
			...
			$map['_string'] = "concat(p.title,p.content) like '%".$soso."%'";
...
*/
	...
	public function publishList(){
		    ...
		    //2016-04-09加入判断查询我的发布或者其他发布
            $is_other = I('post.is_other')?I('post.is_other'):I('get.is_other');
            //2016-05-06加入是否显示超出安卓客户端int范围的数据,临时兼容旧版本,后续会去掉
            $show_long_int = I('post.show_long_int')?I('post.show_long_int'):I('get.show_long_int');
			...
			if(!empty($token)){
                ...
                if(!empty($uerContent)){
                    $where = ' p.zhuangtai = 1 and (p.power = 2 or p.userid= '. $uerContent['0']['id'] .' ) ';
                }
			}
			if(!empty($status)){
			...
			if(!empty($userid)){
				...
				$Searchthe = M('Searchthe');
				foreach ($arr as $k=>$v) {
					$where .= " and p.title like '%$k%' ";

					$Slist = $Searchthe->where("name = '$k'")->limit('1')->select();
					$Sdata = array();
					if (empty($Slist)) {
						$Sdata['name'] = $k;
						$Sdata['addtime'] = time();
						...
						$Sdata['num'] = $Slist['0']['num'] + 1;
						$Searchthe->where("name = '$k'")->data($Sdata)->save();
					}
				}

			}else{
					if(!empty($renzheng)){
						$where .= " and u.renzheng > '0' ";
					}
					//判断城市
					if(!empty($area1)){
						$where .= " and p.area1 = '$area1' ";
						if(!empty($area2)){
							$where .= " and p.area2 = '$area2' ";
						}
					}elseif(!empty($area2)){
						$where .= " and p.area2 = '$area2' ";
					}
				...
				if($status=='1'){
					...
					if(!empty($craftid)){
						$craftid = trim($craftid,',');
						$where .= " and c.cateid in($craftid) ";
						$inner .= " INNER JOIN zj_publishdevice AS c ON c.publishid = p.id ";
					...
					if(!empty($craftid)){
						$craftid = trim($craftid,',');
						$where .= " and FIND_IN_SET($craftid,p.craftid) ";
						$inner .= " INNER JOIN zj_publishmold AS c ON c.publishid = p.id ";
					...
					if(!empty($craftid)){
						$craftid = trim($craftid,',');
						$where .= " and c.cateid in($craftid) ";
						$inner .= " INNER JOIN zj_publishalcor AS c ON c.publishid = p.id ";
					...
					if($num=='10'){
						...
						$num = explode(',',$num);
						$where .= " and p.num between $num[0] and $num[1] ";
					}
				//判断价格
				if ($status == '4') {
					if(!empty($price)) {
						if ($price === '500') {
							...
							$where .= " and p.price >= '" . $price['0'] . "' ";
							if ($price['1'] == '0') {
								$where .= " and p.price2 = 0 ";
							} else {
								$where .= " and p.price2 != 0 and p.price2 <= '" . $price['1'] . "' ";
							}
						}
					}
				}elseif($status == '2'){
					if(!empty($price)) {
						if ($price === '1000000') {
							...
							$price = explode(',', $price);
							//临时解决app int值越界bug
							if(empty($show_long_int)){
								$max_price = (string)$price[1] >= 2147483647 ? 2147483647 : $price[1];
							}else{
								$max_price = (string)$price[1];
							}
//							$where .= " and p.price >= $price[0] and p.price <= $price[1]";
							$where .= " and p.price >= $price[0] and p.price <= $max_price";
						}
					} else {
						if(empty($show_long_int)){
                            //临时解决app int值越界bug
                            $where .= " and p.price <= '2147483647' ";
                        }
                    }
				} elseif (empty($status)) {
					if(empty($show_long_int)){
...
//					}
			...
			//查询数据
			$Publish = M();
			$sql = "SELECT p.*,p.shopname as owner,u.id as uid,u.rterrace,u.renzheng,u.type as stype,u.hxname,u.nickname FROM zj_publish as p
					INNER JOIN zj_user AS u 
					ON u.id = p.userid 
					$inner
					WHERE $where group by p.id desc order by addtime desc limit $limit";
	...
	public function rentOutList($arr){
		...
		foreach ($arr as $key => $value){
            $root[$key]['owner'] = (string)$value['owner'];
			...
			$root[$key]['hxname'] = (string)$value['hxname'];
