...
class UserModel extends Model {
	...
	//环信注册
	public function huanxinRegister($hxname,$nickname){
		...
		$EasemobCount = $Easemob->openRegister(array("username"=>$hxname,"password"=>'JianZhu_5F3A64',"nickname"=>"$nickname"));
	...
	//环信注册
	public function nickname($hxname,$nickname){
		import("ORG.Huanxin.Recson");
		//固定
		$options['client_id'] = 'YXA6t0o2oJpVEeW1DLXywiX-lw';
		$options['client_secret'] = 'YXA6eLIbiKUovCYcNGAZZlxk7zoORPk';
		...
		$options['app_name'] = 'janzhu';
		$Easemob=new Easemob($options);
		$EasemobCount = $Easemob->nickname(array("username"=>$hxname,"nickname"=>"$nickname"));
		$EasemobCount = json_decode($EasemobCount,true);
		if(!empty($EasemobCount['duration'])){
			return $hxname;
		}else{
