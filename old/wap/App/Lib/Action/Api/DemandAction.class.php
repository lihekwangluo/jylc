...
class DemandAction extends IndexAction{
	...
	public function publish(){
		...
		if(empty($t)){
			return get_obj_array('20000','发布太过频繁，请稍后！','');
		}
		$Publish = M('Publish');
		//修改判断
		if(!empty($pid)){
			...
			if(empty($PContent)){
				return get_obj_array('20000','修改数据不存在','');
			}
		}
		//添加类型判断
		if($status == 1){
			...
			return get_obj_array('20000','非法请求！','');
		}
...
//					}
			...
			if(!empty($list)){
				...
				return get_obj_array('20000','未查询到更多数据','');
			}	
	...
	public function workTeamList($arr){
		...
		foreach ($arr as $key => $value){
			...
			$root[$key]['price'] = (string)round($value['price']/10000,2);
	...
	public function appChange($arr){
		...
		if($arr['status'] == 1){
			...
			$str = "工程总价：".$arr['price']." 万";

		}elseif($arr['status'] == 3){
			$str = "队伍：".$arr['num']." 人";
		}elseif($arr['status'] == 4){
			if($arr['price']==0 & $arr['price2']==0){
				$str.="待遇：".面议;
			}else{
	...
	public function reason(){
		    ...
		    if(empty($data['publishid']) || empty($data['publishid']) || empty($data['content'])){
		    	...
		    	if(!empty($list)){
		    		$Public = D('Public');
					$uerContent = $Public->userTokenCark($token);
					if(is_array($userTokenCark)) $data['userid'] = $uerContent['0']['id'];
					$data['addtime'] = time();
					$Reason = M('Reason');
					$Reason->add($data);
					// $root = array();
		    		...
		    		return get_obj_array('20000','举报信息不存在','');
		    	}
