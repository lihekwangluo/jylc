...
<title><if condition="$status eq 1">工友圈<else/>子女教育</if> — 建遇良才</title>
...
<div class="mobile" style="position:relative">
  ...
  <header>
    <div class="header"> <a class="new-a-back" href="javascript:history.back();"> <span><img src="{$_static_}/images/iconfont-fanhui.png"></span> </a>
      <h2><if condition="$status eq 1">工友圈<else/>记工</if></h2> 
    </div>
  </header>
  <!--header 结束-->
  <div class="tp">
    ...
    <volist name="list" id="vl">
  	<div class="tp1">
    	<div><a href="{$_root}/workmate/detail/id/{$vl.wid}">
        <div style="position: relative;" > 
            <table style="height:80px;">
                <tr>
                    <td rowspan="2" style="width:80px;height:80px">
                        <img src="{$vl.upic}" style="width:100%;"/>
                    </td>
                    <td><span style=" margin-left:10px; display:block; font-size:14px; margin-bottom:30px;">{$vl.content|mb_substr=###,0,20,'utf-8'}</span></td>
                </tr>
                <tr>                 
                    <td><span style="margin-left:10px;color:#999;font-size:14px;">{$vl.nickname}&nbsp;&nbsp;&nbsp;&nbsp;{$vl.addtime}</span>  </td>
                </tr>
            </table>   
  ...
  <div class="bl_more" style="text-align: center" id="more"><a href="javascript:;" onclick="get_content()">加载更多</a></div> 
  </if>
...
function get_content(){
  var status = {$vo.status|default="0"};
  $.ajax({
    url:"{$_root}workmate",
    type: 'post', 
